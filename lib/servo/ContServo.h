...
public:
  ...
  ContServo(PinName pin, float max_speed, float max_pulse_ms,
            float min_pulse_ms);
...
protected:
  float m_max_speed, // MAXIMUM ROTATION SPEED in ANGLES PER SECOND (from -max
                     // speed to + max speed),  only valid for CONTINUOUS_SERVO
                     // types

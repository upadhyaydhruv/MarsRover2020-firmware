...
public:
   ...
   * low when backwards [NOT USED BY SERVOMOTOR CLASS]
   * @param freqInHz  Output PWM frequency, default 1kHz [NOT USED BY SERVOMOTOR
   * CLASS]
   * @param inverted  If true, then forward speed will set dir to 0 instead of
   * 1, otherwise inverse
   * @param limit     Maximum speed magnitude
   */
  ServoMotor(PinName pwm, bool inverted = false, float min_pulsewidth_ms = 1.0,
             float max_pulsewidth_ms = 2.0, float limit = 1.0);

  ServoMotor(Motor::t_motorConfig motorConfig);

  /** Set the speed of the motor
   *
   * @param percentage The speed of the motor as a normalised value between -1.0
   * and 1.0
   */

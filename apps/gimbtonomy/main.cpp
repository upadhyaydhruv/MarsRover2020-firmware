...
• Tilt: HS-422 (https://www.robotshop.com/en/hitec-hs-422-servo-motor.html)
• Roll: SG90 (https://datasheetspdf.com/pdf/791970/TowerPro/SG90/1)
*/
ServoMotor panServoMotor(SRVO_PWM_CR, false, 2.1, 0.9,
                         44);  // 44 RPM (264 deg/sec) at 4.8V, max->2100us PW, min->900us PW.
LimServo tiltServo(SRVO_PWM_HS, 180, 2.1, 0.9);
...
void rxCANProcessor() {
  ...
  float pan_pos = 0.0, pan_speed = 0.0, tilt_pos = 0.0, roll_pos = 0.0;
  ...
  while (true) {
    if (can1.read(rxMsg)) {
      switch (rxMsg.id) {
        ...
        case CANID::SET_GIMBAL_TILT_POS:
          rxMsg.getPayload(tilt_pos);
          tiltServo.setPosition(tilt_pos);
          break;

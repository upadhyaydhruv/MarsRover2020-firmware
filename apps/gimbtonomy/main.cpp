...
// Init. Components
// Servos
ContServo panServo(SRVO_PWM_CR, 44.0, 2.1,
                   0.9); // 44 RPM at 4.8V, max->2100us PW, min->900us PW
LimServo pitchServo(SRVO_PWM_MG, 180, 2.1, 0.9);

Encoder::t_encoderConfig panEncoderConfig{
    ENC_PWM_GIMB, NC, NC, NC, NC, NC, NC, 360.0, 0, Encoder::x2_encoding};
...
void move() {
  while (true) {
    event_flags.wait_any(MOVE_FLAG); // moveThread does not sleep, but only
                                     // executes if signaled by can processor

    // Set pitch pos
    pitchServo.setPosition(
        *((float *)(pos_data.get().value.p))); // Cast float to evt ptr and
                                               // dereference float ptr
    ...
    // Set pan pos - stops panServo once it is within tolerance of set position
    // while traveling at designated speed
    float target_pan_pos = (*((float *)(pos_data.get().value.p))),
          cur_pan_pos = panEncoder.getAngle_Degrees();

    if ((cur_pan_pos >= target_pan_pos - POS_TOL) &&
        (cur_pan_pos <= target_pan_pos + POS_TOL))
      panServo.setSpeed(0);
  }
}

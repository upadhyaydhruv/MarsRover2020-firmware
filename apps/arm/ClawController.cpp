...
ClawController::ClawController(t_actuatorConfig actuatorConfig, Motor &motor,
                               Encoder &encoder, DigitalIn &limSwitchMax,
                               AnalogIn &forceSensor, LimServo &tooltipServo,
                               float tooltipExtendedAngle_Degrees,
                               float tooltipRetractedAngle_Degrees,
                               float calibrationTimeout_Seconds)
    : ActuatorController(actuatorConfig, motor, encoder, NULL_DIGITAL_IN,
                         limSwitchMax),
      r_forceSensor(forceSensor), r_tooltipServo(tooltipServo),
...
mbed_error_status_t ClawController::setGapVelocity_CmPerSec(float cmPerSec) {
  float shaftVelocity_DegreesPerSec =
      convertGapVelocityCmToShaftVelocityDegrees(cmPerSec);
  mbed_error_status_t err_status = MBED_ERROR_INVALID_OPERATION;
...
mbed_error_status_t ClawController::extendToolTip() {
  mbed_error_status_t err_status =
      r_tooltipServo.setPosition(m_tooltipExtendedAngle_Degrees)
          ? MBED_SUCCESS
...
mbed_error_status_t ClawController::retractToolTip() {
  mbed_error_status_t err_status =
      r_tooltipServo.setPosition(m_tooltipRetractedAngle_Degrees)
          ? MBED_SUCCESS
          : MBED_ERROR_INVALID_OPERATION;
  return err_status;
}
...
float ClawController::getGapVelocity_CmPerSec() {
  return convertShaftVelocityDegreesToGapVelocityCm(
      getVelocity_DegreesPerSec());
}
...
float ClawController::convertShaftPositionDegreesToGapCm(
    float shaftPosition_Degrees) {
  return (6.363885761e-7) * (shaftPosition_Degrees * shaftPosition_Degrees) -
         (8.793434733e-3) * shaftPosition_Degrees + 15.80749897;
}

float ClawController::convertShaftVelocityDegreesToGapVelocityCm(
    float shaftPosition_DegreesPerSec) {
...
float ClawController::convertGapCmToShaftPositionDegrees(float gap_cm) {
  ...
  return (1.573564198) * (gap_cm * gap_cm) - (158.4968661) * gap_cm +
         2119.701587;
  // return (8.282382533e-3)*(gap_cm*gap_cm*gap_cm*gap_cm) -
  // (2.986760459e-1)*(gap_cm*gap_cm*gap_cm) + (5.007842722)*(gap_cm*gap_cm) -
  // (171.560244)*gap_cm + 2127.848743; // High precision
}

float ClawController::convertGapVelocityCmToShaftVelocityDegrees(
    float gap_cmPerSec) {
  // TODO: fix magic numbers @ALEX
  return 2 * (1.573564198) * gap_cmPerSec - 158.4968661;
  // return 4*(8.282382533e-3)*(gap_cmPerSec*gap_cmPerSec*gap_cmPerSec) -
  // 3*(2.986760459e-1)*(gap_cmPerSec*gap_cmPerSec)
